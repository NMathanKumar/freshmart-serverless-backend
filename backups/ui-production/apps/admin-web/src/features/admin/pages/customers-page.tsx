...
const CustomerState = ({ onRetry, state }: { onRetry?: () => void; state: 'empty' | 'error' | 'loading' }) => state === 'loading' ? <AdminResourceState className="customer-table-state" loadingLabel="Loading customers" rows={4} state="loading" /> : state === 'error' ? <AdminResourceState className="customer-table-state" errorDescription="Try loading the customer list again." errorTitle="Customers could not be loaded" icon={UsersRound} onRetry={onRetry} state="error" /> : <AdminResourceState className="customer-table-state" emptyTitle="No customers found" icon={UsersRound} state="empty" />;
