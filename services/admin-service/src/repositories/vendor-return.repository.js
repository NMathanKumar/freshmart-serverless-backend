...
const ENTITY_TYPE = 'RETURN_TO_VENDOR';
...
const createReturn = async (data, context = {}) => {
  ...
  // Using genId('RTV') as instructed
  const returnId = genId('RTV');

  const item = {
    ...key(returnId),
    adminItemId: returnId,
    ...
    gsi2sk: `UPDATED#${now}`,
    // Custom GSIs for Return to Vendor
  ...
  // Add custom indexing for checking uniqueness if returnNumber exists
  if (data.returnNumber) {
    item.gsi3sk = `RETURN_NUM#${data.returnNumber}`;
  }
...
const checkReturnNumberExists = async (supplierId, returnNumber) => {
  const result = await documentClient.send(
    new QueryCommand({
      TableName: tableName,
      IndexName: 'gsi3',
      KeyConditionExpression: 'gsi3pk = :supplierPk AND gsi3sk = :returnNumSk',
      ExpressionAttributeValues: {
        ':supplierPk': `SUPPLIER#${supplierId}`,
        ':returnNumSk': `RETURN_NUM#${returnNumber}`,
      },
      Limit: 1,
    })
  );
...
const saveReturn = async (returnId, data, currentStatus, expectedVersion, context = {}) => {
  const now = new Date().toISOString();
  ...
  let gsi3sk = `UPDATED#${now}`;
  if (data.returnNumber) {
    gsi3sk = `RETURN_NUM#${data.returnNumber}`;
  }
...
const listReturns = async ({ supplierId, purchaseOrderId, status, page = 1, limit = 20 }) => {
  let command;
  if (supplierId) {
