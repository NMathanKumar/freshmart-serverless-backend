...
export class InventoryService {
  async listInventory(params: InventoryListParams = {}): Promise<InventoryModel[]> {
    try {
      ...
      try {
        ...
        rawItems = response?.data || (response as any)?.items || (Array.isArray(response) ? response : []);
      } catch (err) {
      ...
      // 3. Map SDK `InventorySummary` or real-time `catalogProducts` to UI `InventoryModel`
      let items: InventoryModel[] = [];

      if (rawItems && rawItems.length > 0) {
        items = rawItems.map((inv, index) => {
        ...
        items = catalogProducts.map((p, index) => {
          const currentStock = typeof p.stock === 'number' ? p.stock : 100;
          let mappedStatus: 'IN_STOCK' | 'LOW_STOCK' | 'OUT_OF_STOCK' = 'IN_STOCK';
          if (currentStock === 0) mappedStatus = 'OUT_OF_STOCK';
          else if (currentStock <= 10) mappedStatus = 'LOW_STOCK';

          return {
            id: p.id || `PROD_${index}`,
            name: p.name || 'Product',
            sku: p.sku || `SKU-${index}`,
            category: p.category || DEFAULT_CATEGORIES[index % DEFAULT_CATEGORIES.length],
            warehouse: 'WH-MAIN',
            stock: currentStock,
            maxStock: 150,
            reserved: 0,
            unit: p.unit || 'units',
            status: mappedStatus,
            image: p.image || 'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=80&auto=format&fit=crop&q=80',
      ...
      Logger.warn('Unhandled exception in listInventory', { error: topErr });
      return [];
    }
  }
  ...
  async updateInventory(productId: string, stock: number, _minStock?: number, _unit?: string): Promise<InventoryModel> {
    return this.updateStock(productId, stock);
  }

  async deleteInventory(_id: string): Promise<void> {
    // Delete operation
  }
  ...
  async getInventoryHistory(productId: string, _params: MovementListParams = {}): Promise<MovementModel[]> {
    const movements = await this.listMovements();
    return movements.filter((m: any) => m.productId === productId);
  }

  async adjustDamage(productId: string, payload: AdjustmentPayload): Promise<InventoryModel> {
    return this.adjustStock(productId, payload);
  }
  ...
  async approveAdjustment(productId: string, _movementId: string, _remarks?: string): Promise<InventoryModel> {
    return this.getInventory(productId);
  }

  async rejectAdjustment(productId: string, _movementId: string, _remarks?: string): Promise<InventoryModel> {
    return this.getInventory(productId);
  }
