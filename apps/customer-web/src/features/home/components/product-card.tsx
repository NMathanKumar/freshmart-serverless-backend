    ...
    <Card className="home-product-card group w-[220px] sm:w-[230px] md:w-[240px] flex-none overflow-hidden rounded-[22px] border border-[#e2ebdE] bg-white p-0 shadow-[0_2px_10px_rgba(0,0,0,0.03)] transition-all duration-300 hover:shadow-xl hover:border-[#bdcaba]/70 relative">
      <div className="relative h-44 overflow-hidden bg-[#f4fcf0]/80">
        ...
        <button
          ...
          className="absolute right-3 top-3 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 shadow-xs backdrop-blur-md transition-all hover:scale-110 active:scale-95 cursor-pointer"
        ...
        <Link className="block h-full w-full cursor-pointer" to={`/products/${product.productId}`}>
      ...
      <div className="space-y-2 p-3.5 md:p-4">
        <div>
          <Link className="cursor-pointer" to={`/products/${product.productId}`}>
            <h3 className="truncate text-sm font-extrabold leading-tight text-[#171d16] group-hover:text-[#006b2c] transition-colors">{product.name}</h3>
          </Link>
          <p className="text-xs font-semibold leading-tight text-[#6e7b6c] mt-1">{product.quantity}</p>
        </div>
        <div className="flex items-center justify-between pt-1">
          <div className="flex flex-row items-baseline gap-1.5">
            <span className="text-lg font-black tracking-tight text-[#006c4a]">{formatCurrency(product.price)}</span>
            {product.originalPrice && <span className="text-xs font-bold text-[#8b9888] line-through">{formatCurrency(product.originalPrice)}</span>}
          </div>
          <button
            aria-label={added ? `${product.name} added to cart` : `Add ${product.name} to cart`}
            className="home-add-button flex h-9 w-9 items-center justify-center rounded-full bg-[#006b2c] text-white shadow-sm transition-all hover:bg-[#00873a] hover:scale-110 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#006b2c] focus-visible:ring-offset-2 disabled:opacity-60 cursor-pointer"
            disabled={request.isLoading}
            onClick={() => void add()}
            type="button"
          >
            {request.isLoading ? <LoaderCircle aria-hidden="true" className="h-4 w-4 animate-spin" /> : added ? <Check aria-hidden="true" className="h-4 w-4" /> : <Plus aria-hidden="true" className="h-4 w-4" />}
          </button>
        </div>
