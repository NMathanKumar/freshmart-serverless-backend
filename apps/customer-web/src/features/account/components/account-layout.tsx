...
const sidebarItems = [
  ...
  {
    href: `${customerRoutePaths.settings}#notifications`,
    icon: Bell,
    label: 'Notifications',
  },
  {
    href: `${customerRoutePaths.settings}#appearance`,
    icon: Palette,
    label: 'Appearance',
  },
  {
    href: `${customerRoutePaths.settings}#regional`,
    icon: Settings,
    label: 'Language & Region',
  },
  { href: '/privacy-security', icon: Shield, label: 'Privacy & Security' },
  {
    href: `${customerRoutePaths.settings}#accounts`,
    icon: LockKeyhole,
...
export const AccountShell = ({
  ...
  active: 'settings' | 'security' | 'system';
  children: ReactNode;
}) => {
  ...
  return (
    <div className="account-page min-h-screen bg-[#f4fcf0] pb-20 text-[#171d16] md:pb-0">
      <header className="commerce-glass fixed inset-x-0 top-0 z-50 flex h-16 items-center justify-between border-b border-[#bdcaba] bg-white/80 px-4 shadow-sm md:h-20 md:px-10">
        <Link
          className="commerce-focus text-2xl font-bold tracking-[-0.01em] text-[#006b2c]"
        ...
        <nav className="hidden items-center gap-8 md:flex">
          <Link
            className="text-sm font-semibold text-[#3e4a3d] hover:text-[#006b2c]"
            to={customerRoutePaths.home}
          >
            Shop
          </Link>
          ...
          <Link
            className="border-b-2 border-[#006b2c] py-1 text-sm font-semibold text-[#006b2c]"
            to={
              active === 'security'
                ? '/privacy-security'
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <Link
            ...
            className="commerce-focus relative rounded-full p-2 text-[#006b2c] hover:bg-[#eff6ea]"
            title="Notifications"
            to="/notifications"
          >
            <Bell className="h-5 w-5" />
            <span className="absolute top-0 right-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#a72d51] px-1 text-[10px] font-extrabold text-white shadow-sm">
              3
            </span>
          </Link>
          <Link
        ...
        <button
          ...
          className="commerce-focus rounded-full p-2 md:hidden"
          type="button"
        >
            ...
            <nav className="space-y-1" aria-label="Settings navigation">
              {sidebarItems.map((item) => {
                ...
                const isActive =
                  ...
                  (active === 'settings' &&
                    item.href.startsWith(customerRoutePaths.settings) &&
                    item.href.includes('#') === false);
                return (
                  <Link
                    className={`commerce-focus flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-colors ${isActive ? 'bg-[#d8f4ce] text-[#006b2c]' : 'text-[#3e4a3d] hover:bg-[#eff6ea]'}`}
                    key={item.label}
                    to={item.href}
                  >
              ...
              <div className="mt-4 border-t border-[#bdcaba] pt-4">
                <button
                  aria-disabled="true"
                  className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-semibold text-[#9aa59a]"
                  disabled
                  title="Sign out is coming soon in customer settings"
                  type="button"
                >
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-50 flex h-16 items-center justify-around border-t border-[#bdcaba] bg-[#f4fcf0] shadow-lg md:hidden"
      ...
      >
        ...
        <MobileItem
          icon={<Tag />}
          label="Offers"
          to={`${customerRoutePaths.home}#offers`}
        />
        <MobileItem
          active
          icon={<Settings />}
          label={active === 'system' ? 'Support' : 'Support'}
          to={
            active === 'security'
              ? '/privacy-security'
              : customerRoutePaths.settings
...
const MobileItem = ({
  ...
  <Link
    className={`flex flex-col items-center justify-center gap-1 rounded-xl p-2 text-xs font-semibold ${active ? 'text-[#006b2c]' : 'text-[#3e4a3d]'}`}
    to={to}
  >
    {icon}
    <span>{label}</span>
