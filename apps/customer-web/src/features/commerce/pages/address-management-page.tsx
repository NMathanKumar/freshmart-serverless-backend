    ...
    <CommerceShell active="account" title="Addresses">
      <main className="mx-auto max-w-[1440px] px-4 pb-12 pt-28 md:px-10">
        ...
        <section className="mb-16">
          ...
          {!isLoading && !isError && data.length === 0 && <CommerceState description="Add your first delivery address to speed up checkout." icon="empty" title="No saved addresses" />}
          {!isLoading && !isError && data.length > 0 && <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">{data.map((address) => <AddressCard address={address} key={address.addressId} />)}</div>}
        </section>
        <section className="mx-auto max-w-3xl">
          <form className="commerce-card rounded-2xl p-6 md:p-8" onSubmit={(event) => void handleSubmit(save)(event)}>
            <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
              ...
              <Button className="gap-2 rounded-xl bg-[#d8f4ce] text-[#006b2c] shadow-none hover:bg-[#c4efad]" type="button"><Navigation className="h-4 w-4" />Use Current Location</Button>
            </div>
