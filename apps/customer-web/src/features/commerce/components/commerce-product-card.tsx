...
const productUrl = (productId: string) =>
  customerRoutePaths.productDetails.replace(':productId', productId);

export const CommerceProductCard = ({
  product,
  variant = 'grid',
}: {
  ...
  if (variant === 'compact') {
    return (
      <article className="relative min-w-[160px] rounded-xl border border-[#bdcaba]/20 bg-white p-3 shadow-sm">
        <button
          aria-label={
            isLiked
        ...
        >
          <Heart
            className={`h-3.5 w-3.5 transition-colors ${isLiked ? 'fill-rose-500 text-rose-500' : 'text-[#8b9888] hover:text-rose-500'}`}
          />
        </button>
        <Link to={productUrl(product.productId)}>
          <div className="mb-2 h-32 overflow-hidden rounded-lg bg-[#eff6ea]">
            <img
              alt={product.name}
          ...
          <h3 className="truncate text-sm font-semibold text-[#171d16]">
            {product.name}
          </h3>
        </Link>
        <div className="mt-2 flex items-center justify-between">
          <span className="font-bold text-[#171d16]">
            {formatCurrency(product.price)}
          </span>
          <button
            aria-label={`Add ${product.name} to cart`}
            className="commerce-focus flex h-8 w-8 items-center justify-center rounded-lg bg-[#d8f4ce] text-[#2b4c1d] hover:bg-[#006b2c] hover:text-white"
            onClick={() => void add()}
            type="button"
          >
  ...
  return (
    <article
      className={`commerce-card commerce-lift group relative flex h-full flex-col overflow-hidden p-4 ${variant === 'category' ? 'rounded-xl' : 'rounded-xl'}`}
    >
      {product.badge && (
        <span
          className={`absolute top-4 left-4 z-10 rounded-full px-3 py-1 text-[10px] font-bold uppercase ${product.badgeTone === 'fresh' ? 'bg-[#d8f4ce] text-[#2b4c1d]' : 'bg-[#ffd9de] text-[#3f0016]'}`}
        >
          {product.badge}
        </span>
      )}
      ...
      >
        <Heart
          className={`h-4.5 w-4.5 transition-colors ${isLiked ? 'fill-rose-500 text-rose-500' : 'text-[#8b9888] hover:text-rose-500'}`}
        />
      </button>
      ...
      <div className="flex flex-1 flex-col">
        <p className="mb-1 text-xs font-bold tracking-wide text-[#3f6d2a] uppercase">
          {product.brand}
        </p>
        <Link className="commerce-focus" to={productUrl(product.productId)}>
          <h3 className="mb-1 text-xl leading-7 font-semibold text-[#171d16]">
            {product.name}
          </h3>
        </Link>
        <p className="mb-4 flex items-center gap-1 text-xs font-medium text-[#3e4a3d]">
          ...
          reviews) {product.deliveryTime ? `- ${product.deliveryTime}` : ''}
        </p>
        <div className="mt-auto flex items-center justify-between gap-3">
          <div className="flex flex-col">
            <span className="text-xl font-bold text-[#006b2c]">
              {formatCurrency(product.price)}
            </span>
            {product.originalPrice && (
              <span className="text-xs text-[#6e7b6c] line-through">
        ...
        {updateState.isError && (
          <button
            className="mt-2 text-left text-xs font-semibold text-[#93000a] hover:underline"
            onClick={() => void add()}
            type="button"
          >
            Could not add. Retry
...
export const CartItemCard = ({ item }: { item: CartLine }) => {
  ...
  return (
    <article className="commerce-card flex flex-col gap-4 p-4 sm:flex-row">
      <img
        alt={item.name}
        className="h-32 w-full flex-shrink-0 rounded-xl object-cover sm:w-32"
        loading="lazy"
        src={item.imageUrl}
      />
      <div className="flex flex-1 flex-col justify-between">
        <div className="flex justify-between gap-4">
          <div>
            <h3 className="text-xl font-semibold text-[#171d16]">
              {item.name}
            </h3>
            <p className="text-sm font-semibold text-[#3e4a3d]">{item.brand}</p>
          </div>
          <span className="text-xl font-semibold text-[#006b2c]">
            {formatCurrency(item.price)}
          </span>
        </div>
        <p className="mt-3 flex items-center gap-1 text-xs font-semibold text-[#3f6d2a]">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center rounded-full bg-[#eff6ea] p-1">
            <button
              ...
              className="commerce-focus flex h-8 w-8 items-center justify-center rounded-full hover:bg-white"
              disabled={updateState.isLoading}
          ...
          <div className="flex gap-4">
            <button
              aria-disabled="true"
              className="flex items-center gap-1 text-sm font-semibold text-[#9aa59a]"
              disabled
            ...
            <button
              className="commerce-focus flex items-center gap-1 text-sm font-semibold text-[#93000a] hover:opacity-80"
